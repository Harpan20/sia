...
<body>
    ...
    <table class="mb-fix">
        ...
        ?>
        <?php 
        if($totalDebit->nominal != $totalKredit->nominal){ ?>
        <tr>
            <td colspan="4" class="text-center"><b>Jumlah Total</b></td>
            <td class="text-danger"><b><?= 'Rp. '.number_format($totalDebit->nominal,0,',','.') ?></b></td>
            <td colspan="2" class="text-danger"><b><?= 'Rp. '.number_format($totalKredit->nominal,0,',','.') ?></b></td>
        </tr>
        ...
        <tr>
            ...
            <td class="text-success"><b><?= 'Rp. '.number_format($totalDebit->nominal,0,',','.') ?></b></td>
            <td colspan="2" class="text-success"><b><?= 'Rp. '.number_format($totalKredit->nominal,0,',','.') ?></b></td>
        </tr>
    ...
    <table class="mb-fix" style="margin-bottom:10px;margin-bottom:10px">
        ...
        ?>
            <tr class="text-center">
                ...
                <?php 
                    if($data[$i][$j]->d_or_k=='D'){
                ?>
                <td><?= 'Rp. '.number_format($data[$i][$j]->nominal,0,',','.') ?></td>
                <td>Rp. 0</td>
                <?php 
                    }else{
                ?>
    ...
    <?php endfor; ?>
        ...
        <?= date('d').' '.bulan(date('m')).' '.date('Y').' Pukul '.date('H:i:s')?></p>
</body>
</html>